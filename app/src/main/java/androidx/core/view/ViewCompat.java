...
public class ViewCompat {
    ...
    public static void addOnUnhandledKeyEventListener(View view, OnUnhandledKeyEventListenerCompat onUnhandledKeyEventListenerCompat) {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            object = new View.OnUnhandledKeyEventListener(onUnhandledKeyEventListenerCompat){
    ...
    public static WindowInsetsCompat dispatchApplyWindowInsets(View view, WindowInsetsCompat windowInsetsCompat) {
        ...
        if (Build.VERSION.SDK_INT >= 21 && (windowInsets = windowInsetsCompat.toWindowInsets()) != null && !view.dispatchApplyWindowInsets(windowInsets).equals((Object)windowInsets)) {
    ...
    private static int getAvailableActionIdFromResources(View object) {
        Object object2 = -1;
        List<AccessibilityNodeInfoCompat.AccessibilityActionCompat> list = ViewCompat.getActionList(object);
        for (int i = 0; i < ((View)(object = (Object)ACCESSIBILITY_ACTIONS_RESOURCE_IDS)).length && object2 == -1; ++i) {
            View view = object[i];
            boolean bl = true;
            for (int j = 0; j < list.size(); ++j) {
                boolean bl2 = list.get(j).getId() != view;
                bl &= bl2;
            }
            if (!bl) continue;
            object2 = view;
    ...
    static void notifyViewAccessibilityStateChangedIfNeeded(View view, int n) {
        block4: {
            ...
            block3: {
                ...
                n2 = ViewCompat.getAccessibilityPaneTitle(view) != null ? 1 : 0;
                if (ViewCompat.getAccessibilityLiveRegion(view) != 0 || n2 != 0 && view.getVisibility() == 0) break block3;
                if (view.getParent() == null) break block4;
                try {
    ...
    public static WindowInsetsCompat onApplyWindowInsets(View view, WindowInsetsCompat windowInsetsCompat) {
        WindowInsets windowInsets;
        if (Build.VERSION.SDK_INT >= 21 && (windowInsets = windowInsetsCompat.toWindowInsets()) != null && !(view = view.onApplyWindowInsets(windowInsets)).equals((Object)windowInsets)) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            view.setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener(onApplyWindowInsetsListener){
                ...
                public WindowInsets onApplyWindowInsets(View view, WindowInsets object) {
                    ...
                    return this.val$listener.onApplyWindowInsets(view, (WindowInsetsCompat)object).toWindowInsets();
                }
            });
        }
    }
    ...
    static abstract class AccessibilityViewProperty<T> {
        ...
        boolean booleanNullToFalseEquals(Boolean bl, Boolean bl2) {
            boolean bl3;
            boolean bl4 = false;
            boolean bl5 = bl == null ? false : bl;
            if (bl5 == (bl3 = bl2 == null ? false : bl2)) {
                bl4 = true;
            }
            return bl4;
        }
    ...
    static class UnhandledKeyEventManager {
        ...
        private View dispatchInOrder(View view, KeyEvent keyEvent) {
            ...
            if (viewGroup != null && viewGroup.containsKey(view)) {
        ...
        private void recalcViewsWithUnhandled() {
            ...
            if ((arrayList = sViewsWithListeners).isEmpty()) {
                return;
            }
            synchronized (arrayList) {
                if (this.mViewsContainingListeners == null) {
                    object = new WeakHashMap();
                    this.mViewsContainingListeners = object;
                }
                for (int i = arrayList.size() - 1; i >= 0; --i) {
                    object = sViewsWithListeners;
                    View view = (View)((WeakReference)((ArrayList)object).get(i)).get();
                    if (view == null) {
                    ...
                    while (object instanceof View) {
                        this.mViewsContainingListeners.put((View)object, Boolean.TRUE);
        ...
        static void registerListeningView(View view) {
            ...
            synchronized (arrayList) {
                WeakReference<View> weakReference = arrayList.iterator();
                do {
                    if (weakReference.hasNext()) continue;
                    ArrayList<WeakReference<View>> arrayList2 = sViewsWithListeners;
                    weakReference = new WeakReference<View>(view);
                    arrayList2.add(weakReference);
                    return;
                } while (weakReference.next().get() != view);
                return;
            }
        }
        ...
        static void unregisterListeningView(View view) {
            ArrayList<WeakReference<View>> arrayList = sViewsWithListeners;
            synchronized (arrayList) {
                ArrayList<WeakReference<View>> arrayList2;
                for (int i = 0; i < (arrayList2 = sViewsWithListeners).size(); ++i) {
                    if (arrayList2.get(i).get() != view) continue;
                    arrayList2.remove(i);
                    return;
                }
                return;
