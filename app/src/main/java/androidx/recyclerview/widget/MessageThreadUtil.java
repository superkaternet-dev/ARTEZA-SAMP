...
implements ThreadUtil<T> {
                    ...
                    public void run() {
                        block6: while (true) {
                            ...
                            switch (syncQueueItem.what) {
                                ...
                                case 4: {
                                    this.this$1.val$callback.recycleTile((TileList.Tile)syncQueueItem.data);
                                    continue block6;
                                }
                                case 3: {
    ...
    static class MessageQueue {
        ...
        void removeMessages(int n) {
            synchronized (this) {
                SyncQueueItem syncQueueItem;
                while ((syncQueueItem = this.mRoot) != null && syncQueueItem.what == n) {
                    syncQueueItem = this.mRoot;
                    this.mRoot = syncQueueItem.next;
                    syncQueueItem.recycle();
                }
                SyncQueueItem syncQueueItem2 = this.mRoot;
                if (syncQueueItem2 == null) return;
