...
public final class MemorySizeCalculator {
    ...
    MemorySizeCalculator(Builder builder) {
        ...
        int n3 = builder.screenDimensions.getWidthPixels() * builder.screenDimensions.getHeightPixels() * 4;
        int n4 = Math.round((float)n3 * builder.bitmapPoolScreens);
        int n5 = Math.round((float)n3 * builder.memoryCacheScreens);
        n3 = n2 - n;
        if (n5 + n4 <= n3) {
            this.memoryCacheSize = n5;
            this.bitmapPoolSize = n4;
        } else {
            float f = (float)n3 / (builder.bitmapPoolScreens + builder.memoryCacheScreens);
