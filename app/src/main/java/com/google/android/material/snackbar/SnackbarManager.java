...
class SnackbarManager {
    ...
    private SnackbarManager() {
        this.handler = new Handler(Looper.getMainLooper(), new Handler.Callback(this){
     ...
     * Enabled aggressive exception aggregation
     */
    public void show(int n, Callback object) {
        Object object2 = this.lock;
        synchronized (object2) {
            if (this.isCurrentSnackbarLocked((Callback)object)) {
                this.currentSnackbar.duration = n;
                this.handler.removeCallbacksAndMessages((Object)this.currentSnackbar);
                this.scheduleTimeoutLocked(this.currentSnackbar);
                return;
            }
            if (this.isNextSnackbarLocked((Callback)object)) {
                this.nextSnackbar.duration = n;
            } else {
                SnackbarRecord snackbarRecord;
                this.nextSnackbar = snackbarRecord = new SnackbarRecord(n, (Callback)object);
            }
            object = this.currentSnackbar;
            if (object != null && this.cancelSnackbarLocked((SnackbarRecord)object, 4)) {
                return;
            }
            this.currentSnackbar = null;
            this.showNextSnackbarLocked();
