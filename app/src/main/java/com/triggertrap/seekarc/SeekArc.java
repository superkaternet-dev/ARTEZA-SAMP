...
import com.triggertrap.seekarc.R;
...
extends View {
    ...
    public SeekArc(Context context, AttributeSet attributeSet) {
        ...
        this.mAngleOffset = -90;
        this.init(context, attributeSet, R.attr.seekArcStyle);
    }
    ...
    private void init(Context context, AttributeSet attributeSet, int n) {
        ...
        int n2 = resources.getColor(R.color.progress_gray);
        int n3 = resources.getColor(R.color.default_blue_light);
        ...
        if (attributeSet != null) {
            if ((attributeSet = (context = context.obtainStyledAttributes(attributeSet, R.styleable.SeekArc, n, 0)).getDrawable(R.styleable.SeekArc_thumb)) != null) {
                this.mThumb = attributeSet;
            }
            n = this.mThumb.getIntrinsicHeight() / 2;
            n6 = this.mThumb.getIntrinsicWidth() / 2;
            this.mThumb.setBounds(-n6, -n, n6, n);
            this.mMax = context.getInteger(R.styleable.SeekArc_max, this.mMax);
            this.mProgress = context.getInteger(R.styleable.SeekArc_progress, this.mProgress);
            this.mProgressWidth = (int)context.getDimension(R.styleable.SeekArc_progressWidth, (float)this.mProgressWidth);
            this.mArcWidth = (int)context.getDimension(R.styleable.SeekArc_arcWidth, (float)this.mArcWidth);
            this.mStartAngle = context.getInt(R.styleable.SeekArc_startAngle, this.mStartAngle);
            this.mSweepAngle = context.getInt(R.styleable.SeekArc_sweepAngle, this.mSweepAngle);
            this.mRotation = context.getInt(R.styleable.SeekArc_rotation, this.mRotation);
            this.mRoundedEdges = context.getBoolean(R.styleable.SeekArc_roundEdges, this.mRoundedEdges);
            this.mTouchInside = context.getBoolean(R.styleable.SeekArc_touchInside, this.mTouchInside);
            this.mClockwise = context.getBoolean(R.styleable.SeekArc_clockwise, this.mClockwise);
            this.mEnabled = context.getBoolean(R.styleable.SeekArc_enabled, this.mEnabled);
            n5 = context.getColor(R.styleable.SeekArc_arcColor, n2);
            n6 = context.getColor(R.styleable.SeekArc_progressColor, n3);
            context.recycle();
        }
        n3 = this.mProgress;
        int n7 = this.mMax;
    ...
    private void updateProgress(int n, boolean bl) {
        ...
        this.mProgress = n;
        this.mProgressSweep = (float)n / (float)n2 * (float)this.mSweepAngle;
    ...
    protected void onMeasure(int n, int n2) {
        ...
        this.mThumbXPos = (int)(d * d2);
        d2 = this.mArcRadius;
        d = Math.sin(Math.toRadians(n3));
        Double.isNaN(d2);
        this.mThumbYPos = (int)(d2 * d);
